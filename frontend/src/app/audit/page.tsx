  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl  font-bold text-primary mb-2">Audit Trail</h1>
      ...
      <div className="flex items-center space-x-4 mb-6">
        ...
        <div className="flex items-center space-x-2 text-sm text-muted-foreground">
          ...
          <span className="text-status-healthy text-sm bg-status-healthy/10 px-2 py-0.5 rounded border border-status-healthy/30">VERIFIED</span>
        </div>
      </div>
                ...
                {events.map((evt, idx) => {
                  ...
                  return (
                    <tr key={evt.id} className="hover:bg-primary/5 transition-colors">
                      <td className="px-6 py-4 text-sm text-xs text-muted-foreground whitespace-nowrap">{evt.timestamp}</td>
                      <td className="px-6 py-4 text-sm text-[10px] text-primary/70">0x{hash}</td>
